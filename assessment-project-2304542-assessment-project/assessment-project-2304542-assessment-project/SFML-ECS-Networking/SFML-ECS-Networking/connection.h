...
struct Connection
{
	...
	sf::Vector2f InterpolateProjectiles(float dt) {
		// Calculate interpolation step
		float step = 0.f;
		...
		if (message_history.size() > 1) { // Make sure we have at least 2 messages for Linear interpolation.
			// Calculate new position by interpolating between last two known positons.
			return (message_history[1].projectilePosition + (message_history[0].projectilePosition - message_history[1].projectilePosition) * inter_t);

		}
		else
			return message_history[0].projectilePosition; // Just return the last known position.
